...
data:
	docker compose run --rm freqtrade download-data --trading-mode futures -c user_data/exchange-config.json -p BTC/USDT:USDT ETH/USDT:USDT BNB/USDT:USDT --timerange 20200101- --timeframe 5m 15m 1h 

buy-hyperopt:
	docker compose run --rm freqtrade hyperopt -c user_data/config.json -c user_data/exchange-config.json -j 3 --timerange 20200101- --timeframe 1h --timeframe-detail 15m --hyperopt-loss SortinoHyperOptLossDaily -s SimpleFutures -e 500 --spaces buy
