...
buy-hyperopt:
	docker compose run --rm freqtrade hyperopt -c user_data/config.json -c user_data/exchange-config.json -j 3 --timerange 20220901- --timeframe 1h --timeframe-detail 5m --hyperopt-loss SortinoHyperOptLossDaily -s SimpleFutures -e 100 --spaces buy

sell-hyperopt:
	docker compose run --rm freqtrade hyperopt -c user_data/config.json -c user_data/exchange-config.json -j 3 --timerange 20220901- --timeframe 1h --timeframe-detail 5m --hyperopt-loss SortinoHyperOptLoss -s SimpleFutures -e 100 --spaces sell
